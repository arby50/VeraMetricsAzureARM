...
CALL az vm create --resource-group %NEW_RESOURCE_GROUP_NAME% --name %NEW_VM_NAME% --attach-os-disk %DISK_NAME% --size Standard_B2s --os-type Linux --os-disk-delete-option Delete --nic-delete-option Delete --security-type TrustedLaunch --enable-secure-boot true --enable-vtpm true --output none
...
if %errorlevel% equ 0 (
    ...
    set IMAGE_DEFINITION=VeraMetricsEngineImageDefinition
    echo Found image: !IMAGE_DEFINITION! in gallery: !GALLERY_NAME! in resource group: !GALLERY_RESOURCE_GROUP!
) else (
    ...
    if %errorlevel% leq 3 (
        ...
        set IMAGE_DEFINITION=VeraMetricsEngine
        echo Found image: !IMAGE_DEFINITION! in gallery: !GALLERY_NAME! in resource group: !GALLERY_RESOURCE_GROUP!
    ) else (
...
echo Getting latest version from gallery image definition: %IMAGE_DEFINITION%
for /f %%i in ('az sig image-version list --resource-group %GALLERY_RESOURCE_GROUP% --gallery-name %GALLERY_NAME% --gallery-image-definition %IMAGE_DEFINITION% --query "max([].name)" -o tsv') do set LATEST_VERSION=%%i
